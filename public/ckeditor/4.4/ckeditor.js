			...
			fire: function() {
				...
				return function(h, m, j) {
					...
					if (p) {
						var t = p.listeners;
						if (t.length)
							for (var t = t.slice(0), y, n = 0; n < t.length; n++) {
								if (p.errorProof) try {
									y =
			...
			extend: function(a) {
				...
				for (var c = 1; c < f; c++) {
					var e = arguments[c],
						o;
					for (o in e)
						if (d === true || a[o] == null || o == 'createIterator')
							if (!b || o in b) a[o] = e[o]
				}
				return a
			},
	...
	(function() {
		...
		CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
			...
			removeStyle: function(a) {
				...
				if (!d.removeProperty && (a == "border" || a == "margin" || a == "padding")) {
					...
					for (var d = [], e = 0; e < b.length; e++)
						if (c)
							for (var j = 0; j < c.length; j++) d.push([a, b[e], c[j]].join("-"));
			...
			copyAttributes: function(a, b) {
				for (var c = this.$.attributes, b = b || {}, e = 0; e < c.length; e++) {
					var m = c[e],
						...
						p;
					if (!(j in b))
						if (j == "checked" && (p = this.getAttribute(j))) a.setAttribute(j, p);
			...
			forEach: function(a, b, c) {
				...
				if (e !== false)
					for (var c = this.getChildren(), m = 0; m < c.count(); m++) {
						e = c.getItem(m);
		...
		var c = function(a) {
			...
			f = function(a, b, c, d) {
				...
				for (c = l; c < r.length; c++) {
					...
					if (!o[c] || g.$.parentNode != o[c].$.parentNode)
			...
			enlarge: function(a, b) {
				...
				switch (a) {
					...
					case CKEDITOR.ENLARGE_ELEMENT:
							...
							for (; l;) {
								...
								if (l.type == CKEDITOR.NODE_COMMENT) l = l.getPrevious();
									...
									if (l.type == CKEDITOR.NODE_TEXT) {
										...
										(CKEDITOR.env.webkit ? 1 : 0) || b && l.is("br")) && !l.data("cke-bookmark"))
										if (x && CKEDITOR.dtd.$removeEmpty[l.getName()]) {
											g = l.getText();
											if (d.test(g)) l = null;
											else
												for (var j = l.$.getElementsByTagName("*"), m = 0, F; F = j[m++];)
						...
						if (s.type == CKEDITOR.NODE_TEXT)
							if (CKEDITOR.tools.trim(s.substring(j)).length) x = true;
							else {
								x = !s.getLength();
								if (j == s.getLength()) {
									if (!(l = s.getNext())) k =
			...
			shrink: function(a, b, c) {
				if (!this.collapsed) {
					...
					if (f && f.type == CKEDITOR.NODE_TEXT)
						if (o)
							if (o >= f.getLength()) d.setStartAfter(f);
							else {
								d.setStartBefore(f);
								i = 0
	...
	(function() {
		...
		function c(a, b, c) {
			if (!a.match || a.match(b)) {
				...
				b = b.classes;
				if (a) {
					for (var d = false, e = a === true, i = b.length; i--;)
						if (e || a(b[i])) {
							b.splice(i, 1);
							d =
								true
		...
		function y(a, b, d, f) {
			...
			if (f.doCallbacks && a.elementCallbacks) {
				a: for (var l = a.elementCallbacks, B = 0, s = l.length, g; B < s; ++B)
					if (g = l[B](b)) {
						k = g;
						break a
					}
		...
		function l(a, b, c) {
			var d = a.name;
			if (N.$empty[d] || !a.children.length)
				if (d == "hr" && b == "br") a.replaceWith(i());
				else {
					a.parent && c.push({
						check: "it",
						el: a.parent
		...
		function x(a, b) {
			...
			for (e in d)
				if (e == "class") {
					c = d[e].split(/\s+/);
					for (q = a.classes.join("|"); o = c.pop();)
						if (q.indexOf(o) == -1) return false
				} else if (a.attributes[e] != d[e]) return false;
			for (i in f)
				if (a.styles[i] != f[i]) return false;
			return true
		}
						...
						a: {
							...
							if (i)
								for (l = 0; l < i.length; ++l)
									if ((n = d.match(i[l])) && n[0].length == d.length) {
										c = true;
										break a
									}
		...
		load: function(a, e, b) {
			...
			CKEDITOR.scriptLoader.load(d, function(a, d) {
				...
				for (var f = 0; f < a.length; f++)
					for (var n = g[a[f]], j = 0; j < n.length; j++) {
						var o = n[j];
						h[o] = this.get(o);
						c[o] = 1
					}
				...
				g = function(b) {
					a.call(this, b, function(a) {
						...
						for (h in a) {
							...
							if (!e[h]) {
								if (u.icons)
									for (var y = u.icons.split(","), n = y.length; n--;) CKEDITOR.skin.addIcon(y[n], u.path + "icons/" + (CKEDITOR.env.hidpi && u.hidpi ? "hidpi/" : "") + y[n] + ".png");
								e[h] = 1
							}
							if (t) {
								t.split && (t = t.split(","));
		...
		function p(a) {
			...
			CKEDITOR.plugins.load(c.split(","), function(c) {
				...
				for (var i in c) {
					...
					if (r && (A = r.match(l)))
						for (; r = A.pop();) CKEDITOR.tools.setTimeout(function(a, b) {
					...
					if (a(f)) o.push(f);
						...
						if (d == "pre") l = true;
							...
							for (;;) {
								...
								if (!f.isUnknown && !i.isUnknown && !h[d])
									if (i.isOptionalClose) n.onTagClose(g);
				...
				return function(e) {
					if (e.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
						var i = [],
							...
							q, o;
						if (l)
							for (A(l, 1) && i.push(l); l;) {
								if (d(l) && (q = c(l)) && A(q))
									if ((o = c(q)) && !d(o)) i.push(q);
									else {
										k(r).insertAfter(q);
										q.remove()
									}
						...
						if (i = (typeof f == "function" ? f(e) : f) !== false)
							if (!r && !CKEDITOR.env.needsBrFiller &&
						...
						this.attachListener(a, "key", function(b) {
							...
							if (c in l) {
								...
								if (CKEDITOR.env.ie && CKEDITOR.env.version < 11 && (f = b.getSelectedElement()) || (f = d(b))) {
									...
									if ((g = k.block) && (j = g[c ? "getPrevious" : "getNext"](t)) && j.type == CKEDITOR.NODE_ELEMENT && j.is("table") && i[c ? "checkStartOfBlock" : "checkEndOfBlock"]()) {
										a.fire("saveSnapshot");
		...
		var n = function() {
				...
				return function(s, n, u) {
					...
					if (!A.checkReadOnly()) {
							...
							for (q(B, H); z < B.length; z++) {
								...
								if (C) E = z > 0;
									...
									if (V) {
										...
										V = 0
									} else H.insertNode(D.node); if (D.lastNotAllowed && z < B.length - 1) {
						...
						for (V = K.createBookmark(); I = n.zombies.pop();)
							if (I.getParent()) {
								S = K.clone();
						...
						if (U)
							for (; I = U.pop();) CKEDITOR.env.needsBrFiller ? I.appendBogus() : I.append(K.document.createText(" "));
	...
	(function() {
		...
		CKEDITOR.dom.selection.prototype = {
			...
			selectRanges: function(a) {
				...
				this.reset();
				if (b)
					for (var b = this.root, e, f = 0; f < a.length; ++f) {
						e = a[f];
			...
			if (b.collapsed) {
				...
				for (p.getPosition(m) == CKEDITOR.POSITION_FOLLOWING && (p = 0); p;) {
					...
					if (p.equals(m)) {
						...
						if (t && i && CKEDITOR.dtd.$block[r])
							for (var s = p, w = c(s), x = void 0, F = w.length, M = 0, s = F && new CKEDITOR.dom.range(s.getDocument()); M < F; ++M) {
								var x = w[M],
						...
						if (w)
							if ((w = p.getParent()) && ((w.getDtd() || CKEDITOR.dtd.span)[f] || h) && (!g.parentRule || g.parentRule(w))) {
						...
						for (var j = o(this, d), t = j.hasAttributes(), u = q.getCommonAncestor(), r = {}, w = {}, x = {}, F = {}, O, T, R; j && u;) {
							if (u.getName() == f) {
								...
								for (T in g.styles)
									if (!x[T] && (R = u.getStyle(T))) j.getStyle(T) == R ? r[T] = 1 : x[T] = 1
							}
							u = u.getParent()
						}
			...
			createIterator: function() {
				...
				return {
					getNextRange: function(g) {
						...
						if (h && a.length > 1) {
							if (!d)
								for (var m = a.length - 1; m >= 0; m--) e.unshift(a[m].createBookmark(true));
							if (g)
								for (var j = 0; a[d + j + 1];) {
...
*/
	...
	(function() {
		...
		function k() {
			...
			target = filebrowser.target.split(':');

			input = dialog.getContentElement(target[0], target[1]);

			hawkejs.scene.emit('filebrowser', input, dialog, filebrowser, null);
	...
	(function() {
		function k(a) {
			...
			} catch (g) {}(CKEDITOR.env.gecko || CKEDITOR.env.ie && "CSS1Compat" == e.document.$.compatMode) && this.attachListener(this,
				"keydown", function(a) {
					var b = a.data.getKeystroke();
					if (b == 33 || b == 34)
	...
	(function() {
		...
		function $(a) {
			...
			else return null; if (p(e) || ja(a, e, i) || e.getParent().is(aa)) return null;
			var h = r(a, e, !i);
			if (h) {
	...
	(function() {
		function j(a) {
			var b = null != a.config.autosave_SaveKey ? a.config.autosave_SaveKey : "autosave_" + window.location,
				d = null != a.config.autosave_NotOlderThan ? a.config.autosave_NotOlderThan : 1440,
				c = null != a.config.autosave_saveOnDestroy ? a.config.autosave_saveOnDestroy : !1,
				e = null != a.config.autosave_saveDetectionSelectors ? a.config.autosave_saveDetectionSelectors : "a[href^='javascript:__doPostBack'][id*='Save'],a[id*='Cancel']";
			CKEDITOR.scriptLoader.load(CKEDITOR.getUrl(CKEDITOR.plugins.getPath("autosave") +
				...
				if (localStorage.getItem(b)) {
					var f = h(b),
						c = f.data,
						f = f.saveTime;
					a.getData() == c ? localStorage.removeItem(b) : moment(new Date).diff(f, "minutes") > d ? g(b) : (c = a.lang.autosave.loadSavedContent.replace("{0}", moment(f).lang(a.config.language).format(a.lang.autosave.dateFormat)), confirm(c) ? a.openDialog("autosaveDialog") : g(b))
				}
			});
			...
			a.on("uiSpace", function(a) {
				"bottom" == a.data.space && (a.data.html +=
					'<div class="autoSaveMessage" unselectable="on"><div unselectable="on" id="cke_autoSaveMessage_' + a.editor.name + '"class="hidden">' + a.editor.lang.autosave.autoSaveMessage + "</div></div>")
			}, a, null, 100)
		}
				...
				return {
					...
					contents: [{
						...
						elements: [{
							...
							items: [
								[a.lang.autosave.sideBySide, "sideBySide"],
								[a.lang.autosave.inline, "inline"]
							],
							"default": "sideBySide",
							onClick: function() {
								m(this._.dialog, a, b)
		...
		function l(a, b) {
			var d = LZString.compressToUTF16(JSON.stringify({
				data: b.getData(),
				saveTime: new Date
			}));
			localStorage.setItem(a, d);
			var c = document.getElementById("cke_autoSaveMessage_" + b.name);
			c && (c.className = "show", setTimeout(function() {
				c.className = "hidden"
			}, 2E3))
		}
		...
		function m(a, b, d) {
			var d = h(d),
				c = difflib.stringAsLines(b.getData()),
				e = difflib.stringAsLines(d.data),
				f = (new difflib.SequenceMatcher(c, e)).get_opcodes();
			a.getContentElement("general", "diffContent").getElement().setHtml('<div class="diffContent">' + diffview.buildView({
				...
				newTextName: b.lang.autosave.autoSavedContent + moment(d.saveTime).lang(b.config.language).format(b.lang.autosave.dateFormat) + "'",
				contextSize: 3,
				viewType: "inline" ==
					a.getContentElement("general", "diffType").getValue() ? 1 : 0
			}).outerHTML + "</div>")
		}
		if (function() {
